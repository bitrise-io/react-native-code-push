...
+ (nullable instancetype)manifestFromJSON:(NSDictionary *)json error:(NSError **)error
{
    ...
    if (versionValue != nil && ![versionValue isKindOfClass:[NSNull class]]) {
        if (![versionValue isKindOfClass:[NSNumber class]]) {
            if (error) *error = [CodePushErrorUtils errorWithMessage:[NSString stringWithFormat:@"Diff manifest field \"version\" must be a number, but is \"%@\"", versionValue]];
            return nil;
        }
    ...
    if (version < 2 && patchedFiles.count > 0) {
        if (error) *error = [CodePushErrorUtils errorWithMessage:[NSString stringWithFormat:@"Diff manifest version %ld does not support binary diff patches, but the manifest lists %lu of them", (long)version, (unsigned long)patchedFiles.count]];
        return nil;
    }
