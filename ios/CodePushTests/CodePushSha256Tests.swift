...
final class CodePushSha256Tests: XCTestCase {
    ...
    func testHexForFile_largerThanChunkSize_matchesExpectedDigest() throws {
        // Deterministic payload larger than the implementation's 8 KiB read-chunk
        // boundary, so this exercises the multi-chunk CC_SHA256_Update loop rather
        // than only its first iteration.
        let payload = Data((0..<(8 * 1024 + 173)).map { UInt8($0 % 256) })
        let url = try writeFile(named: "large.dat", contents: payload)
        let expectedHex = SHA256.hash(data: payload)
