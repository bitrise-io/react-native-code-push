...
final class CodePushBinaryDiffPatcherTests: XCTestCase {
    ...
    func testApply_pathTraversalInPatchedFilesKey_isRejected() throws {
        ...
        XCTAssertThrowsError(
            try CodePushBinaryDiffPatcher.applyBinaryDiffPatches(
                ...
                currentPackageFolder: currentPackageFolder.path,
                unzippedFolder: unzippedFolder.path,
                newUpdateFolder: newUpdateFolder.path)) { error in
            let message = (error as NSError).localizedDescription
            XCTAssertTrue(message.contains("escapes"), message)
        }
    }

    func testApply_emptyPatchedFilesKey_isRejectedAsMalformed() throws {
        try installBasicFixtures()
        let baseHash = try hashOfFixture("basic/old.dat")
        let targetHash = try hashOfFixture("basic/new.dat")
        ...
        XCTAssertThrowsError(
            try CodePushBinaryDiffPatcher.applyBinaryDiffPatches(
                manifest: manifest(baseHash: baseHash, targetHash: targetHash, relativePath: ""),
                currentPackageFolder: currentPackageFolder.path,
                unzippedFolder: unzippedFolder.path,
                newUpdateFolder: newUpdateFolder.path)) { error in
            let message = (error as NSError).localizedDescription
            XCTAssertTrue(message.contains("empty"), message)
            XCTAssertFalse(message.contains("escapes"), message)
        }
    }

    func testApply_missingCurrentPackageFolder_reportsFolderUnusable() throws {
        try FileManager.default.removeItem(at: currentPackageFolder)

        XCTAssertThrowsError(
            try CodePushBinaryDiffPatcher.applyBinaryDiffPatches(
                manifest: manifest(baseHash: "irrelevant-not-reached-before-folder-check", targetHash: "irrelevant-not-reached-before-folder-check"),
                currentPackageFolder: currentPackageFolder.path,
                unzippedFolder: unzippedFolder.path,
                newUpdateFolder: newUpdateFolder.path)) { error in
            let message = (error as NSError).localizedDescription
    ...
    func testApply_fileToPatchMissing_reportsTheReadFailure() throws {
        // Only the patch is installed: the manifest names a file the current package does not have.
        try FileManager.default.copyItem(
            at: fixtureURL("basic/patch.bsdiff"),
            to: unzippedFolder.appendingPathComponent(patchRelativePath))
        let baseHash = try hashOfFixture("basic/old.dat")
        ...
        XCTAssertThrowsError(
            try CodePushBinaryDiffPatcher.applyBinaryDiffPatches(
                ...
                newUpdateFolder: newUpdateFolder.path)) { error in
            let message = (error as NSError).localizedDescription
            XCTAssertTrue(message.contains("Could not open file for reading"), message)
            XCTAssertFalse(message.contains("mismatch"), message)
        }
    }
